...
func fetchHTTP(resource *url.URL, options *Options) (string, error) {
	...
	if dry.FileIsDir(dest) {
		...
		re := regexp.MustCompile(`(?m)filename="?([\w-\.]+)"?`)
		if re.MatchString(contentDisposition) {
			dest = filepath.Join(dest, filepath.Base(re.FindStringSubmatch(contentDisposition)[1]))
	...
	if options.Progress {
		log.Println("Fetching", resource.String(), "to", dest)
	...
	// NOTE: destTmpWriter and copyWriter may actually be the same thing here if Options.Progress is
	// false.
